...
void prep_robot(void* data)
{
	...
	while (1)
	{
		...
		if ((char*)data == "A") {
			...
			printf("Erreur lors du transfert de données vers les tâches des robots.");
			exit(1);
		}
		errMsg(err, "Error while trying to access q_prep_a or q_prep_b");

		printf("TACHE PREP_ROBOT_%s COMMANDE #%d @ %d : Début préparation robot %s.\n", (char*) data, orderNumber, OSTimeGet() - startTime, (char*) data);
		OSTimeDly(*robotPrepTime);
		printf("TACHE PREP_ROBOT_%s COMMANDE #%d @ %d : Fin préparation robot %s.\n", (char*) data, orderNumber, OSTimeGet() - startTime, (char*) data);
		...
		printf("TACHE PREP_ROBOT_%s COMMANDE #%d @ %d : Demande de démarrage du transport.\n", (char*) data, orderNumber, OSTimeGet() - startTime);
